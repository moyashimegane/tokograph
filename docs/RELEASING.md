...
Changes land on main via pull requests (squash merge). CI runs on every PR
and on main; the PR title becomes the release-notes line, so write it as a
user-facing change description.

Release checklist (one release per merged issue):
...
- [ ] Release notes: auto-generated from PR titles; add a manual summary line if they are unclear
